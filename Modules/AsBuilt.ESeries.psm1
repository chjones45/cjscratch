...
# ChangeLog:
# 2026.08.20.14 - Fail immediately when the initial storage-system inventory request fails, preventing SSID prompts and misleading empty reports after authentication errors.
# 2026.08.19.13 - Backported PowerShell 7 TLS handling, secure legacy credentials, JSON replay, effective switch defaults, DOCX soft-break recovery, resilient Pandoc checks, metadata fallbacks, and API failure report parity from the StorageGRID collector.
$ScriptVersion = "2026.08.20.14"
...
function Convert-DocxSoftBreakTokens {
    ...
    try {
        ...
        $documentXmlText = $reader.ReadToEnd()
        $reader.Dispose()
        if ($documentXmlText.IndexOf($Token, [System.StringComparison]::Ordinal) -lt 0) { return }
