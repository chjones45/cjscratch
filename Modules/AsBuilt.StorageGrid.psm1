...
function Invoke-StorageGridSantricityApplianceCollection {
    param(
        ...
        [Parameter(Mandatory = $false)][string]$DocxDirectory,
    ...
    foreach ($candidate in @($Candidates)) {
        ...
        foreach ($ip in @($candidate.CandidateIps)) {
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace($DocxDirectory)) {
                    $invokeParameters.DocxDir = $DocxDirectory
                }
...
# ChangeLog:
# 2026.08.20.62 - Added guided shared-versus-per-appliance SANtricity credential prompting while preserving CLIXML map and non-interactive workflows.
# 2026.08.19.61 - Added tri-state SANtricity collection, NonInteractive mode, post-Grid prompting, explicit opt-out, and hardened appliance collection reporting.
# 2026.08.19.60 - Added PowerShell 7 certificate-bypass request handling when -ValidateCerts is not specified, matching the existing Windows PowerShell 5.1 behavior for self-signed StorageGRID certificates.
# 2026.08.18.59 - Added Grid Regions to the end of the ILM - Storage Configuration section with an N/A fallback.
# 2026.08.18.58 - Added a page break and a References - Online Support Resources section at the end of the report, restoring the reference table from the legacy Ansible/Jinja2 template.
# 2026.08.17.57 - Added Management Interface CORS collection/reporting and switched VLAN Interfaces from Node Id to Node Name.
# 2026.08.17.56 - Restored the Logged Headers subsection heading and Markdown paragraph separation so its table renders correctly between Audit Logging and Audit Syslog - Destination.
# 2026.08.17.55 - Enabled DOCX window autofit for the merged Grid Untrusted Client Network Nodes table while preserving the vertically merged Default Mode cell.
# 2026.08.17.54 - Replaced Untrusted Node Count with resolved Untrusted Client Network Nodes, mapped from node IDs to node names, sorted by Grid Site and Node Name, and rendered Default Mode as a vertically merged cell.
# 2026.08.17.53 - Parsed PEM certificates in Grid Management Certificate and Grid Storage API Certificate into common name, SAN, and UTC expiry details. Grid Manager and Tenant Manager global Load Balancer entries now resolve to the shared management server certificate when endpoint-specific metadata is unavailable.
# 2026.08.17.52 - Enabled DOCX window autofit for the merged Grid Load Balancer Certificate table while preserving vertical SAN-row merges.
# 2026.08.17.51 - Removed Endpoint ID and Pinned HA Groups count from Grid Load Balancer Endpoint Config; added endpoint-to-HA-group mapping. Redesigned Grid Load Balancer Certificate to map endpoint names to common names, separate SAN rows with merged metadata columns, and UTC-formatted dd-MM-yyyy expiry dates.
# 2026.08.17.50 - Moved Grid Passwords to directly before Health Summary in the report; removed the previous duplicate position under Configuration - Access Control.
# 2026.08.17.49 - Renamed internal functions to approved PowerShell verbs: Initialize-Path, Set-DocxTableStyle, Get-OrCreate-XmlChildNode, Get-OrCreate-CoreElement, Set-CustomStringProperty, ConvertTo-NormalizedApiPayload, and Write-StorageGridMarkdown. Updated all call sites without changing script parameters or report behavior.
# 2026.08.17.48 - Removed the inactive _Phase2Placeholder_StorageGridSections function containing obsolete E-Series report-generation code; active StorageGRID rendering and collection paths are unchanged.
# 2026.08.17.47 - Standardized comments for every static active StorageGRID report section and subsection, removed obsolete table-number references, and aligned comment names with current headings.
# 2026.08.17.46 - Added Client Networks to the Node Network Routes table. Client Network route subnets, including 0.0.0.0/0 default routes, are now shown with soft-return separation.
# 2026.08.17.45 - Restored CIDR notation in the Node IP Addresses Grid, Admin, and Client columns; route tables already retain CIDR notation.
# 2026.08.17.44 - Split Node Networks into Node IP Addresses (Grid/Admin/Client IP columns) and Node Network Routes (Node, Site, Grid Networks, Admin Networks). IP columns now contain address values without CIDR prefix lengths; route columns retain CIDR subnets with soft-return separation.
# 2026.08.17.43 - Configured the merged Grid Appliance Shelves table to use DOCX window autofit while retaining vertically merged Node cells.
# 2026.08.14.42 - Reorganized report TOC groups around the documented StorageGRID 11.9-12.1 Grid Manager areas: Overview, Dashboard, Nodes, Support, Configuration (Access Control, Monitoring and Logging, System and Storage, Security and Certificates, Network, Firewall and Appliance Security, Client Access and Load Balancing), Alerts, ILM, and Tenants. Moved Grid Passwords into Access Control while retaining its content unchanged.
# 2026.08.13.40 - Added report sections for previously-collected-but-unrendered data: Grid Health Summary, Grid Custom Alert Rules, Grid Alert Silences, Grid Client Certificates, Grid Single Sign-On, Grid External Load Balancers, Grid Firewall (Blocked/External Ports, Privileged IPs), Grid Ciphers, Grid BMC. Each new section includes descriptive text explaining the StorageGRID feature's purpose. Removed the dead sg_cgr_advanced collection (endpoint, fact, and variable) since it had no remaining report usage.
# 2026.08.13.41 - Removed Alarms rows from Grid Health Summary; merged Node cells in Grid Appliance Shelves; removed Policy Id column from and renamed Grid Traffic Classification Policies; renamed Grid Alert Receiver to Alert Setup; added N/A fallback row to Cloud Storage Pool Details when no cloud storage pools are configured; removed the API Mapping Gaps section entirely. Expanded the descriptive text for every active report section (Document Version Information through Grid Account Usage - Bucket Details) with content grounded in the StorageGRID and StorageGRID Appliance public documentation.
# 2026.08.13.39 - Removed Versioning Suspended, Encryption, and Quota from Grid Account Usage - Bucket Details tables.
# 2026.08.13.38 - Added Grid Account Usage - Bucket Details per-account section (Bucket Name, Object Count, Data Capacity, Region, Consistency, Versioning Enabled/Suspended, Encryption, Quota).
# 2026.08.13.37 - Grid Account Usage: removed Account Id and Calculation Time; Object Count now uses comma-separated thousands formatting; table sorted by Data Capacity descending, then Account Name alphabetically.
# 2026.08.13.36 - Fixed the merged-cell table emitter's column widths being discarded: tables now declare an explicit dxa tblW/tblLayout=fixed and per-cell tcW; Apply-DocxTableStyle now detects fixed-layout tables, skips the autofit-to-window pct conversion for them only, and re-asserts their per-cell dxa widths (Pandoc was normalizing tcW to auto on these tables). All other tables keep the existing autofit-to-window behavior unchanged.
# 2026.08.13.35 - Grid Account Details now uses fixed-width Field/Value columns (consistent across all tenants) instead of per-table autofit; fixed the merged-cell table emitter to render embedded soft-returns as real line breaks instead of losing them.
# 2026.08.13.34 - Converted Grid Alert Receiver to a 2-column Field/Value table per receiver (removed Receiver Id), with Recipients spanning merged rows; added Username and From Address (N/A placeholders) and CA Bundle/Client Cert/Client Key Present indicators.
# 2026.08.13.33 - Removed Account Id/Capabilities/Use Account Identity Source from Grid Account summary; added Grid Account Details per-tenant section (Capabilities soft-return, Use own identity source, Allow Platform Services, Allow S3 Select, Grid Federation Connections, Storage Quota, S3 Object Lock compliance/retention settings).
# 2026.08.13.32 - Grid ILM Rule Usage, Grid ILM Rule, and ILM Rule Details now all use one shared alphabetically-sorted rule list, so the Rule Details section order matches the Usage/Rule table order.
# 2026.08.13.31 - Grid ILM Rule table: removed Reference Time and Ingest Behavior (now only in ILM Rule Details); moved Compliance Compatible next to Rule Name and renamed to Compliant.
# 2026.08.13.30 - Removed API field from Grid ILM Rule table.
# 2026.08.13.29 - Sorted Grid ILM Rule Usage table alphabetically by Rule Name.
# 2026.08.13.28 - Storage Pools Criteria field now separates every Site/Grade/Site Group value with a soft-return instead of semicolons, consistent with the soft-return separation already used between multiple disk records.
# 2026.08.13.27 - Renamed Storage Pools heading to Grid Logical Storage and ILM Storage Pool table to Storage Pools; removed Pool Id and switched Criteria (formerly Disk Layout) to soft-return; simplified Grid Cloud Storage Pool to a name-only summary plus new per-pool Cloud Storage Pool Details section; removed Rule Id from Grid ILM Rule Usage and switched Used By Policies to soft-return; fixed Placements table to repeat Retention After on every row of a placement instead of blanking it after the first.
# 2026.08.13.26 - Resolved ILM Policy Activated By user IDs to names (fallback "N/A - Unresolved User ID"); added Grid ILM Policy Tags table; enhanced Storage Pools section with Grid ILM Grade and Grid ILM Grade Site Counts tables and resolved pool disk layout site/grade IDs to names.
# 2026.08.13.25 - Added expanded ILM Rule Details and ILM Policy Details sections (merged-cell filter/placement/rule breakdown per rule and policy); removed Policy Id from Grid ILM Policy and sorted it by Active (desc) then Policy Name; removed resolved ILM placement gap note; fixed Convert-ToArrayPayload return-unwrapping bug that produced spurious empty rows for single-item array properties.
# 2026.08.13.24 - Added DDTP/RDMD/SFSN/SFID/SANM/SAIP/SAIQ to the private node-attribute collector; added Drive Type and RAID Mode to Grid Appliance Storage, plus new Grid Appliance Storage Controller and Grid Appliance Shelves tables.
# 2026.08.13.23 - Fixed Used (%) formatting across all Grid/Site/Node capacity tables to always show 2 decimal places instead of trimming trailing zeros.
# 2026.08.13.22 - Condensed the Grid Capacity Total/Object/Metadata tables into a single table with a Capacity Type column (Overall Grid, Objects, Metadata).
# 2026.08.13.21 - CustomerName/CustomerLocation/ProjectName custom document properties are now always created, using a placeholder value when not supplied, to avoid "Error! Unknown document property name." on the title page.
# 2026.08.13.20 - Merged Grid Node Summary and Grid Node Details tables into a single Grid Node Details table (Node Name, Site, Node Type, Node Role, Platform).
# 2026.08.13.19 - Removed SANtricity Firmware column from Grid Appliance Storage; renamed Grid User Member Of to Grid Groups, now showing Display Name with soft-return separated multi-group cells.
# 2026.08.13.18 - Removed redundant Grid Name column from tables after Grid Configuration Details; split Grid Identity Source into federation/auth tables; Grid Group soft-returns; fixed Grid User Member Of to map Unique Name only on id match; fixed Grid SNMP null counts to show N/A; removed Cross-Grid Replication row from Grid S3 Lock Options; converted Metadata Reserved Space to whole-number TB; removed Drive Detail Records row and stopped sg_drive_details collection.
# 2026.08.13.17 - Converted Grid Appliance Storage Drive Size from raw bytes to decimal TB (1 decimal place).
# 2026.08.13.16 - Switched appliance-node identification from unreliable hasEseries flag to Platform == "SGA" (Grid Appliances filter and node-attribute collector).
# 2026.08.13.15 - Added private /attributes node-hardware collector (Compute Serial CNSN, BMC IP CBMC, Appliance Model SAMD, Drive Size DDSZ) mapped via SSM service id from sg_service_ids; wired into Grid Appliances/Grid Appliance Storage tables.
# 2026.08.13.14 - Switched DOCX soft-break marker to a plain token to survive Pandoc processing before conversion to Word line breaks.
# 2026.08.13.13 - Added DOCX soft-break token conversion to real Word line breaks for table cell values.
# 2026.08.13.12 - Fixed table-cell soft returns by normalizing to <br /> and enabling Pandoc gfm+raw_html input parsing.
# 2026.08.13.11 - Updated soft-return rendering in Grid Node Networks and Grid Sites members; renamed appliance serial columns to Compute Serial and Storage Serial.
# 2026.08.13.10 - Updated Grid Configuration Details table to render DNS servers, NTP servers, and Grid Networks with soft returns.
# 2026.08.13.9 - Enforced explicit N/A fallback for appliance metric-derived fields when data is missing/unavailable.
# 2026.08.13.8 - Updated appliance Prometheus label mapping to match observed SG 11.9 metric keys (BundleDisplayVersion, CFW, NVSRAM, chassis_serial_number).
# 2026.08.13.7 - Refined appliance metric parsing for model candidates and updated mapping-gap notes to reflect Prometheus-backed appliance storage fields.
# 2026.08.13.6 - Updated StorageGRID report mappings: split appliance tables, added Grid Node Networks table, adjusted Grid Sites columns, and added appliance Prometheus metric collection (firmware/version/NVSRAM/serial/model candidates).
# 2026.08.13.5 - Added backward-compatible secure password parameters (ApiPasswordSecure + legacy alias) and retained legacy conversion path.
# 2026.08.13.4 - Focused cleanup: removed unused assignments, preserved default-on switch behavior without default=true declarations, and hardened legacy ApiPassword handling.
# 2026.08.13.3 - Fixed SG 11.9 object-capacity total/% by deriving object total as data used + usable space.
$ScriptVersion = "2026.08.20.62"
...
function Repair-DocxSectionHeaders {
    ...
    try {
        ...
        foreach ($nodeName in @('footerReference', 'headerReference', 'pgSz', 'pgMar', 'cols', 'titlePg', 'docGrid')) {
...
function Convert-DocxSoftBreakTokens {
    param(
    ...
    try {
        ...
        $replacement = '</w:t></w:r><w:r><w:br/></w:r><w:r><w:t xml:space="preserve">'
        $updatedXmlText = $documentXmlText.Replace($Token, $replacement)

        $null = $zip.GetEntry('word/document.xml').Delete()
        $newEntry = $zip.CreateEntry('word/document.xml', [System.IO.Compression.CompressionLevel]::Optimal)
        $writer = New-Object System.IO.StreamWriter($newEntry.Open())
        $writer.Write($updatedXmlText)
        $writer.Dispose()
    }
    finally {
        $zip.Dispose()
...
function Set-DocxTableStyle {
    ...
    try {
        ...
        foreach ($table in $tables) {
            ...
            # Repair per-cell widths for fixed-layout tables: Pandoc's DOCX writer can normalize tcW to "auto"
            # even for raw OpenXML passthrough tables, so re-assert each column's dxa width from the tblGrid.
...
function Resolve-PandocExecutable {
    param([Parameter(Mandatory = $true)][string]$DocxDirPath)

    $localPandoc = Join-Path -Path $DocxDirPath -ChildPath "pandoc.exe"
    if (Test-Path -Path $localPandoc) {
        return $localPandoc
    }
    ...
    throw "Pandoc was not found. Place pandoc.exe in .\\docx or install Pandoc in PATH."
}
...
$pandocExe = Resolve-AsBuiltPandocExecutable -DocxDirectory (Resolve-OptionalPath -Path $DocxDir)
$docxReferenceTemplate = Resolve-ExistingFilePath -Path ".\docx\reference-template.docx"
$pandocTableStyleFilter = Resolve-ExistingFilePath -Path ".\docx\pandoc-apply-table-style.lua"

Write-Host "Converting markdown to DOCX ..."
Convert-AsBuiltMarkdownToDocx -MarkdownPath $mdPath -DocxPath $docxPath -PandocExecutable $pandocExe -MarkdownFormat 'gfm+raw_html-autolink_bare_uris+raw_attribute' -ReferenceDocumentPath $docxReferenceTemplate -LuaFilterPath $pandocTableStyleFilter -TableStyleName $DocxTableStyleName -NumberSections ([bool]$EnableDocxNumberSections) -SoftBreakToken 'SGSOFTBREAKTOKENX' -TemporaryFilePrefix 'sg_docx' | Out-Null
...
if (-not [string]::IsNullOrWhiteSpace($docxPath) -and (Test-Path -LiteralPath $docxPath -PathType Leaf)) {
    Repair-AsBuiltDocxSectionHeaders -DocxPath $docxPath
}
